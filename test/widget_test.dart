...
void main() {
  ...
  testWidgets('shot tracer start opens camera-only capture', (
    WidgetTester tester,
  ) async {
    await pumpSwingLensApp(
      tester,
    ...
    await tester.pumpAndSettle();
    await tester.ensureVisible(find.text('START TRACER CAMERA'));
    await tester.tap(find.text('START TRACER CAMERA'));
    await tester.pump();
    await tester.pump(const Duration(milliseconds: 500));

    expect(
      find.byKey(const ValueKey('tracer-camera-only-preview')),
      findsOneWidget,
    );
    expect(
    ...
    expect(find.text('UPLOAD TRACER VIDEO'), findsNothing);
  });
